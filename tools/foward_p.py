...
def forward(img1,img2):
	...
	#print(label_map)
	img1_cut = os.listdir(img1.replace('tcp','middle_dir'))
	for name in img1_cut:
		path = os.path.join(img1.replace('tcp','middle_dir'),name)
		if path.split('.')[-1]=='jpg':
				print(path)
				img = cv2.imread(path)
	...
	img2_cut = os.listdir(img2.replace('tcp','middle_dir'))
	for name in img2_cut:
		path = os.path.join(img2.replace('tcp','middle_dir'),name)
		if path.split('.')[-1]=='jpg':
				print(path)
				img = cv2.imread(path)
				img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
