...
def vis_detections(im, class_name, dets, thresh=0.5, im_name=''):
    ...
    for i in inds:
        ...
        cv2.imwrite(im_name.split('.')[0]+'_'+str(i)+'.jpg',img_cut)
    os.remove(im_name)
    ...
    plt.axis('off')
    plt.tight_layout()
    #plt.savefig(os.path.join('../data/detect_result',im_name))
    #plt.draw()
...
if __name__ == '__main__':
    ...
    entries = os.listdir("all")
    for entry in range(47,49):
        listglob = []
        listglob = glob.glob(os.path.join("all",str(entry),'*.jpg'))
        print(listglob)
        for im_name in listglob:
            ...
            #os.remove(im_name)
    '''listglob = []
    listglob = glob.glob(r"../data/VOCdevkit2007/VOC2007/JPEGImages/*.jpg")
